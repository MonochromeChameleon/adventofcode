...
export class Question extends QuestionBase {
  ...
  part2(input) {
    ...
    const allCubes = Array.from({ length: maxX + 1 - minX }, (_, x) => x + minX)
      .flatMap((x) => Array.from({ length: maxY + 1 - minY }, (_, y) => y + minY)
        .flatMap((y) => Array.from({ length: maxZ + 1 - minZ }, (_, z) => new Vector(x, y, z + minZ))));

    const lava = new Set(input.map((i) => i.toString()));
    const airCubes = allCubes.filter((c) => !lava.has(c.toString()));

    const regions = airCubes.reduce((regions, cube) => {
      const adjoiningRegions = Object.entries(regions).filter(([, r]) => r.some((c) => c.subtract(cube).manhattan === 1)).map(([k]) => k);
      const discreteRegions = Object.keys(regions).filter((k) => !adjoiningRegions.includes(k));

      return Object.fromEntries([
        ...discreteRegions.map((k) => [k, regions[k]]),
        [cube.toString(), this.postParse([cube, ...adjoiningRegions.flatMap((k) => regions[k])])]
      ]);
    }, {});

    const innerRegions = Object.values(regions)
      .filter((r) => r.every(({ x, y, z }) => x !== minX && x !== maxX && y !== minY && y !== maxY && z !== minZ && z !== maxZ));

    const innerTotals = innerRegions.map((region) => region.reduce((tot, cube, ix) => {
