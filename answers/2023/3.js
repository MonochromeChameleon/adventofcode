...
export class Question extends QuestionBase {
  ...
  findNumbers(line, validIndexes) {
    const {
      ...
      sofar: s,
      ...out
    } = line.split('').reduce(({ sofar = '', vix, ...ooo }, c, ix) => {
      if (!/\d/.test(c)) {
  ...
  part1({ grid, adjacencyMap, lines, width }) {
    ...
    const partNumbers = lines.flatMap((line, ix) =>
      this.findNumbers(line, this.getValidDigitIndexes(ix, width, adjacentDigitIndexes)),
    );
  ...
  part2({ grid, adjacencyMap, lines, width }) {
    ...
    const gearsWithNeighbours = maybeGears
      .map((ix) => {
        const adjacent = adjacencyMap[ix]
          .filter((ad) => /\d/.test(grid[ad]))
          .filter((ad, ii, arr) => arr[ii + 1] !== ad + 1);
        return { ix, adjacent };
      })
      .filter(({ adjacent }) => adjacent.length === 2);
    const neighbours = [...new Set(gearsWithNeighbours.flatMap(({ adjacent }) => adjacent))].sort((a, b) => a - b);
    const numbers = lines.flatMap((line, ix) =>
      this.findNumbers(line, this.getValidDigitIndexes(ix, width, neighbours)),
    );
