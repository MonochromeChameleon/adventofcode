...
class Cave {
  ...
  paths({ caves, visited = [], canRepeat = false }) {
    ...
    return this.links.filter(it => caves[it].multivisit || !visited.includes(it) || canRepeat)
      .flatMap(link => {
...
export class Question extends QuestionBase {
  constructor (args) {
    super(12, 226, 5157, 3509, 144309, args);
  }
  ...
  parseInput(lines) {
    const groups = lines.flatMap(this.parseLine)
